...
void transmit(int socket, char* data, int target_size)
{
    int goal = target_size;
    while (target_size>0)
    {
        int size_sent= send(socket , data , target_size , 0);
        if(size_sent<0){
        ...
        target_size -= size_sent;
        data += size_sent;
    }
    printf("Successfully sent %d bytes\n", goal);
}

void receive(int socket, int target_size)
{
...
{   
    ...
    char* data = (char*)malloc(target_size); 
    if (data == NULL) {
    ...
    for(int i=0; i<3; i++){
        ...
        transmit(new_socket, data, target_size);
        //Receive the ack from the client of receving target_size characters, check if the client receive all bytes
        receive(new_socket, target_size);
    }
